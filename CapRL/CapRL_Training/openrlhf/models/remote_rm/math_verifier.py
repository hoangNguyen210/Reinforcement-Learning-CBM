...
def verify_relax(input_queue, output_queue):
    while True:
        ...
        if 1:
            ...
            reward += 0.0 if 1 - np.min(ret2['match']) < anls_threshold else 1 - np.min(ret2['match'])
            print (f'Reward: {reward} ########### {sol} ############ {student_answer}')
        #except Exception:
...
def verify_math_sample(content, sol):
        gold_parsed = parse(
            sol,
...
if __name__ == "__main__":
    ...
    p = Process(target=verify_relax, args=(input_queue, output_queue))
    p.start()
