...
from openrlhf.utils.utils import get_tokenizer, get_vl_processor, timing
from openrlhf.trainer.ppo_utils import DATA_PROCESSOR_MAP
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        if self.args.train_vlm:
            processor = get_vl_processor(pretrain, None, "left", strategy, use_fast=not self.args.disable_fast_tokenizer)
        ...
        self.generate_kwargs.update(dict(gamma=self.args.gamma, lambd=self.args.lambd, pad_token_id=self.tokenizer.pad_token_id, eos_token_id=self.tokenizer.eos_token_id))
    ...
    def fit(
        ...
        for episode in range(episode, args.num_episodes):
            ...
            for _, rand_prompts, labels in self.prompts_dataloader:
                ...
                with timing('Generating samples', logger):
                ...
                with timing('Making experience', logger):
                ...
                with timing('Appending samples', logger):
                    refs = self.actor_model_group.async_run_method_batch(method_name="append", experience=experiences)
                    ray.get(refs)

                with timing('PPO training', logger):
                    status = self.ppo_train(steps)
                    
                if self.args.len_penalty:
                    avg_real_acc = torch.tensor([exp.real_acc.mean() for exp in experiences]).mean().item()
                    status.update({"avg_real_acc": avg_real_acc})
