...
class PolicyModelActor(BaseModelActor):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain, max_steps=None, vllm_engines=None):
        ...
        if args.lr_warmup_steps > 0:
            warm_up_steps = args.lr_warmup_steps
            warm_type = 'Fix'
        else:
            warm_up_steps = math.ceil(max_steps * args.lr_warmup_ratio)
            warm_type = 'Ratio'
        print (f'Warm Up with {warm_up_steps} steps by [{warm_type}] mode.')
