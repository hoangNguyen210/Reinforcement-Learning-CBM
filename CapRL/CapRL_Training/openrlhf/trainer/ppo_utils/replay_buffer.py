...
def split_experience_batch(experience: Experience, data_processor) -> List[BufferItem]:
    """Split a batch of experiences into individual BufferItems."""
    batch_size = len(experience.sequences)
    # Get fields from BufferItem, excluding 'info'
    keys = tuple(field.name for field in fields(BufferItem) if (field.name != "info" and field.name != "visual_inputs"))
    ...
    for i in range(batch_size):
        ...
        item = {key: (getattr(experience, key)[i] if (getattr(experience, key) is not None and key != 'visual_inputs') else None) for key in keys}
        ...
        if data_processor is not None:
            item["visual_inputs"] = visual_inputs_chunks[i]
...
def make_experience_batch(items: List[BufferItem], packing_samples=False, data_processor=None) -> Experience:
    ...
    kwargs = {
        key: (
            zero_pad_sequences([getattr(item, key) for item in items], side, stack=True)
...
def remove_padding_in_sequences(items):
    for item in items:
        ...
        keys = tuple(field.name for field in fields(BufferItem) if (field.name != "info" and field.name != "visual_inputs" and (field.name not in act_keys)))
        for key in keys:
            value = getattr(item, key)
            if value is not None:
                setattr(item, key, value[:right_pad])
        action_mask = item.action_mask
        act_rad_padding = action_mask.long().flip(0).argmax()
        act_rad_padding = None if act_rad_padding == 0 else -act_rad_padding
        for key in act_keys:
            value = getattr(item, key)
            if value is not None:
                setattr(item, key, value[:act_rad_padding])
...
class NaiveReplayBuffer(ABC):
    ...
    def sample(self) -> Experience:
        ...
        experience = make_experience_batch(items, self.packing_samples, self.data_processor)
        if self.cpu_offload:
    ...
    def collate_fn(self, batch) -> Experience:
        if self.dynamic_batch:
            batch = batch[0]
        experience = make_experience_batch(batch, self.packing_samples, self.data_processor)
        return experience
