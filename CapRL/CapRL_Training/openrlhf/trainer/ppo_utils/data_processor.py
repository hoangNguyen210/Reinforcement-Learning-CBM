...
class Qwen2VLDataProcessor(BaseDataProcessor):
    def __call__(
        ...
        batch = processor.apply_chat_template(
            messages, tokenize=True, add_generation_prompt=True, return_dict=True, return_tensors="pt", padding=False, truncation=truncation
        )
    ...
    def split_input_batch(self, batch: Dict) -> List[Dict]:
        ...
        for k, v in batch.items():
            if k in ('prompt', 'output_token_ids'):
                for i in range(batch_size):
                    batch_kwargs[i][k] = v[i]
            elif v is not None:
                keys.append(k)
            else:
                for i in range(batch_size):
        ...
        if "pixel_values" in keys:
            ...
            for i in range(batch_size):
                ...
                if vision_start_num != vision_end_num:
                    print ('ERROR VIS TOKEN NUM:', vision_start_num, vision_end_num)
                    now = datetime.now()
                ...
                if img_num > 1:
                    print ('NUM > 1 is not a correct setting, check the code')
                    img_num = 1
                if img_num == 0:
