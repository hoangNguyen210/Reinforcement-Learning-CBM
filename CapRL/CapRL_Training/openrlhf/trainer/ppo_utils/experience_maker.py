...
from openrlhf.utils.utils import remove_pad_token, zero_pad_sequences, timing
from openrlhf.utils.remote_rm_utils_capvl import remote_rm_fn_ray

logger = init_logger(__name__)


@ray.remote(num_cpus=1)
def process_prompt(data_processor, prompt, max_len):
    msg = data_processor.apply_chat_template(prompt, tokenize=False, add_generation_prompt=True)
    image = data_processor.get_images_from_messages(prompt)
    visual_input = data_processor(prompt, max_len)
    return msg, image, visual_input
...
class Experience:
    ...
    def __init__(
        ...
        self.visual_inputs = visual_inputs or {}
...
class SamplesGenerator:
    def __init__(self, vllm_engines, strategy, tokenizer, data_processor, prompt_max_len):
    ...
    def _generate_vllm(self, raw_all_prompts: List[str], raw_all_labels, **kwargs) -> List[Experience]:
        """Generate samples using vLLM engine.

        Args:
            raw_all_prompts: List of prompts to generate from, length: rollout batch size
            raw_all_labels: List of labels corresponding to prompts
        ...
        if self.args.train_vlm:
            cap_prompt_base = 'Please describe this image in detail.'
            args = self.strategy.args
            n_caps_per_prompt = args.n_caps_per_prompt
            assert args.n_samples_per_prompt % n_caps_per_prompt == 0

            tic_time = time.time()
            cap_prompts = []
            cap_qas = []
            visual_info_ref = []
            for prompt in raw_all_prompts:
                temp = json.loads(prompt)
                assert len(temp) == 3
                temp, answer = temp[:2], temp[2:]
                assert answer[0]['role'] == 'answer'
                qas = eval(answer[0]['content'])
                cap_qas.append(qas)

                temp = temp[1:2]
                temp[0]['content'][1]['text'] = cap_prompt_base
                temp = json.dumps(temp, ensure_ascii=False)
                cap_prompts.append(temp)
                visual_info_ref.append(process_prompt.remote(self.data_processor, temp, self.args.prompt_max_len))
            visual_infos = ray.get(visual_info_ref)  # msg, image, visual_input
            logger.info(f"✨ Process Visual Infos: {time.time()-tic_time:.3f} seconds")
            tic_time = time.time()

            all_prompts = sum([[prompt] * n_caps_per_prompt for prompt in cap_prompts], [])
            all_labels = sum([[label] * n_caps_per_prompt for label in raw_all_labels], [])
            all_visual_info = sum([[visual_info] * n_caps_per_prompt for visual_info in visual_infos], [])
            # Distribute requests to engines and collect responses
            batch_size = (len(all_prompts) + len(llms) - 1) // len(llms)
            refs = []
            for i, llm in enumerate(llms):
                visual_info_batch = all_visual_info[i * batch_size: (i + 1) * batch_size]
                vllm_inputs = []
                for i in range(len(visual_info_batch)):
                    vllm_inputs.append({
                        "prompt": visual_info_batch[i][0][0],
                        "multi_modal_data": {"image": visual_info_batch[i][1][0]},
                    })
                refs.append(llm.add_requests_vlm.remote(sampling_params=sampling_params, vllm_vision_input=vllm_inputs))
            ray.get(refs)
            ...
            logger.info(f"✨ VLLM Generate Samples: {time.time()-tic_time:.3f} seconds")
        else:
        ...
        for i in range(len(all_outputs)):
            ...
            inputs = self.data_processor.processor.apply_chat_template(
                json.loads(prompt),
                tokenize=True,
            ...
            sequences = torch.tensor(inputs + list(output.outputs[0].token_ids))[None, :]
            ...
            # input_ids = inputs + list(output.outputs[0].token_ids)
            ...
            # response_length = len(output.outputs[0].token_ids)
            # action_mask[len(inputs) : len(inputs) + response_length] = 1

            response_text = self.tokenizer.decode(sequences[-response_length:], skip_special_tokens=True)
            ...
            action_mask = action_mask[:truncate_length].to("cpu")
            ...
            if self.args.train_vlm:
                qa = cap_qas[i // self.args.n_caps_per_prompt]
                rollout_samples.answers = [response_text, qa]
                rollout_samples.visual_inputs = all_visual_info[i][2]
...
class RemoteExperienceMaker(ABC):
    ...
    def make_experience(self, samples_list: List[Experience]) -> List[Experience]:
        ...
        if samples_list[0].rewards is not None:
            ...
            queries_list = sum(
                [
                    self.tokenizer.batch_decode(seq, skip_special_tokens=False)
            ...
            answers_list_ = sum([s.answers for s in samples_list], [])
            answers_list = []
            for i in range(len(answers_list_) // 2):
                answers_list.append(answers_list_[i * 2: (i + 1) * 2])

            # debug
            # remote_rm_fn(self.remote_rm_url[0], queries=queries_list, prompts=answers_list, labels = [1] * len(prompts_list))

            if self.args.train_vlm:
                r_refs = remote_rm_fn_ray.remote(self.remote_rm_url[0], queries=queries_list, prompts=answers_list, labels=labels_list)
            else:
        ...
        if args.train_vlm and args.remove_pi_old:
            action_log_probs_ref = ray.put([[torch.zeros_like(s).float()] for s in action_mask_list])
        else:
        ...
        if samples_list[0].rewards is not None:
            ...
            # Get rewards info from remote model
            with timing('Compute rewards', logger):
                rewards_info = ray.get(r_refs)

            if self.args.train_vlm:
                rewards_info = [dict(rewards=torch.Tensor([info])) for info in rewards_info]
            
            # breakpoint()
            # Process rewards and scores
            update_samples_with_rewards(rewards_info, samples_list)
            if self.args.len_penalty:
                for samples in samples_list:
                    samples.real_acc = samples.rewards.clone()
                    if self.args.penalty_type == "all_penal":
                        lengths_tensor = samples.info['response_length'].to(
                            device=samples.rewards.device, 
                            dtype=samples.rewards.dtype
                        )
                        len_penalty_value = self.args.len_penalty * 2 * lengths_tensor
                        samples.rewards -= len_penalty_value
                    elif self.args.penalty_type == "max_penal":
                        lengths_tensor = samples.info['response_length'].to(
                            device=samples.rewards.device, 
                            dtype=samples.rewards.dtype
                        )
                        exceeding_length = torch.clamp(lengths_tensor - 950, min=0)
                        len_penalty_value = self.args.len_penalty * 2 * exceeding_length
                        samples.rewards -= len_penalty_value
        else:
            # Reward Model
        ...
        for i, (samples, action_log_probs, base_action_log_probs, value) in enumerate(
            ...
            if (self.initial_model_group is not None) and (not args.use_kl_loss):
                ...
                kl_mean = torch.zeros((action_log_probs.shape[0],), dtype=action_log_probs.dtype, device=device)
    ...
    def compute_advantages_and_returns(
        ...
        for experience, reward in zip(experiences, rewards):
            # put reward to last token index
