...
class MessageQueue:
    ...
    def __init__(self, config: DictConfig, max_queue_size: int = 1000):
        ...
        # async safe (lazily initialized to avoid Python 3.10 event loop mismatch
        # when created inside a Ray async actor's sync __init__)
        self._lock = None
    ...
    def _ensure_async_primitives(self):
        """Lazily create asyncio Lock/Condition to avoid Python 3.10
        'loop argument must agree with lock' error in Ray async actors.
        Use the reverse-init pattern from PR #4230: create Condition()
        without a lock arg, then extract its internal ._lock."""
