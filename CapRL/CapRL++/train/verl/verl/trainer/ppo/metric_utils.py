...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # reward_fn 返回的分解项（qa/format/长度奖励 R_l 与 cap tokenizer 长度等），便于 wandb 分曲线观察
    for key, sec in (
        ("qa_reward", "reward_decomp/qa_reward"),
        ("format_reward", "reward_decomp/format_reward"),
        ("length_reward", "reward_decomp/length_reward"),
    ):
        if key in batch.non_tensor_batch:
            arr = np.asarray(batch.non_tensor_batch[key], dtype=np.float64)
            metrics[f"{sec}/mean"] = float(np.mean(arr))
            metrics[f"{sec}/min"] = float(np.min(arr))
            metrics[f"{sec}/max"] = float(np.max(arr))
