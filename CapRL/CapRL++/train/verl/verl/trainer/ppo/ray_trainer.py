            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # Diagnostic: log reward/advantage stats（可选，设 VERL_LOG_ADV_STATS=1 开启，并写入本地文件）
                        if (
                            int(os.environ.get("VERL_LOG_ADV_STATS", "0")) != 0
                            and self.global_steps > 0
                            and self.global_steps % 10 == 0
                        ):
                            r = batch.batch["token_level_rewards"].sum(-1).float()
                            a = batch.batch["advantages"].sum(-1).float()
                            r_min, r_mean, r_max, r_std = (
                                r.min().item(),
                                r.mean().item(),
                                r.max().item(),
                                r.std().item(),
                            ...
                            # 控制台打印一份
                            print(
                                f"[VERL adv diagnostic step={self.global_steps}] "
                                f"reward sum: min={r_min:.4f} mean={r_mean:.4f} max={r_max:.4f} std={r_std:.4f} | "
                                f"advantage sum: min={a_min:.4f} mean={a_mean:.4f} max={a_max:.4f} std={a_std:.4f}",
                                flush=True,
                            )

                            # 同时追加写入本地 JSONL 文件，方便事后分析
                            try:
                                log_dir = os.environ.get(
                                    "VERL_ADV_STATS_DIR",
                                    os.environ.get("VERL_LOG_DIR", os.path.join(os.getcwd(), "logs")),
                                )
                                os.makedirs(log_dir, exist_ok=True)
                                log_path = os.environ.get(
                                    "VERL_ADV_STATS_PATH",
                                    os.path.join(log_dir, "adv_stats.jsonl"),
                                )
                                record = {
                                ...
                                print(f"[VERL adv diagnostic] failed to write log file: {e}", flush=True)
