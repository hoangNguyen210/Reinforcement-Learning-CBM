...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        if n_image_features > n_image_tokens:
            ...
            if deepstack_image_embeds:
                deepstack_image_embeds = [e[:n_image_tokens] for e in deepstack_image_embeds]
    ...
    if pixel_values_videos is not None:
        ...
        if n_video_features > n_video_tokens:
            # prompt 被左截断时 input_ids 中视频占位符变少，特征按完整视频生成，需截断对齐
            video_embeds = video_embeds[:n_video_tokens]
            if deepstack_video_embeds:
                deepstack_video_embeds = [e[:n_video_tokens] for e in deepstack_video_embeds]
