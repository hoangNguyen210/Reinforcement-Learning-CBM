...
class RLHFDataset(Dataset):
            ...
            for segment in segments:
                if segment == "<image>":
                    ...
                    if isinstance(image, Image.Image):
                        ...
                        if "bytes" in image:
                            image["image"] = Image.open(BytesIO(image["bytes"]))
                        content_list.append({"type": "image", **image})  # ← 关键修改：使用 **image 展开
                    else:
                    ...
                    if isinstance(video, dict):
                        ...
                        # 数据集里可能是 path/URL 字符串，与 vision_utils 约定一致用 "video" 键
                        content_list.append({"type": "video", "video": video})
                    else:
                        raise TypeError(f"video must be dict or str, got {type(video)}")
                    video_offset += 1
                else:
                    content_list.append({"type": "text", "text": segment})
            message["content"] = content_list
    ...
    def __getitem__(self, item):
        ...
        if "extra_info" not in row_dict or row_dict["extra_info"] is None:
            ...
            row_dict["extra_info"] = dict(row_dict["extra_info"])

        videos = row_dict.get(self.video_key, None)
        if videos:
            if isinstance(videos, str):
                row_dict["extra_info"].setdefault("video_path", videos)
                row_dict["extra_info"].setdefault("video_paths", [videos])
            elif isinstance(videos, list):
                str_videos = [video for video in videos if isinstance(video, str)]
                if str_videos:
                    row_dict["extra_info"].setdefault("video_paths", str_videos)
                    if len(str_videos) == 1:
                        row_dict["extra_info"].setdefault("video_path", str_videos[0])
