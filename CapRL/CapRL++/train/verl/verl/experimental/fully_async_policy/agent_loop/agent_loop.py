...
class FullyAsyncAgentLoopWorker(AgentLoopWorker):
    def __init__(
        ...
        rw_one = reward_loop_worker_handles[0] if reward_loop_worker_handles else None
        # Do not pass load_balancer_handle as reward_router_address (must be str | None).
        super().__init__(config, servers, None, rw_one)
...
class FullyAsyncAgentLoopManager(AgentLoopManager):
    def __init__(
        ...
        super().__init__(
            config, worker_group, None, rollout_resource_pool, reward_loop_worker_handles
        )
